...
exportbuffers_t = [] # TODO: see above
...
def get_RLE(arr):
    values = arr[np.r_[True, arr[1:] != arr[:-1]]]
    lengths = np.diff(np.r_[np.where(np.r_[True, arr[1:] != arr[:-1]])[0], len(arr)])
    return values, lengths
def find_trigger(trace_t, samprate, debug):
    if trace_t[0] != 0:
        print("trigger: doesn't start with 0")
        return None
    if not np.any(trace_t):
        print("trigger: never 1")
        return None
    rle = get_RLE(trace_t)
    if len(rle[0]) != 3:
        print("trigger: cannot find a single clear trigger sequence")
        if debug:
            print(rle[0])
            print(rle[1])
        return None
    found_idx = np.argmax(trace_t) # returns 0 if there are no 1s
    trigger_delay = 2e-6
    trigger_delay_samples = round(samprate * trigger_delay)
    if found_idx < trigger_delay_samples:
        print("trigger: less than delay from start")
        return None
    return found_idx - trigger_delay_samples
        ...
        try:
            while True:
                ...
                if data == CAPTURE_START_CMD:
                    ...
                    # process buffers
                    if debug:
                        print(f"collected {len(exportbuffers)}+{len(exportbuffers_t)} buffers")
                    #print(type(exportbuffers[0]))
                    #print(exportbuffers[0].shape)
                    trace = np.concatenate(exportbuffers).astype(np.float32)
                    trace_t = np.concatenate(exportbuffers_t).astype(np.float32)
                    #print(trace.shape)
                    # send serialized through socket
                    if debug:
                        print(f"searching for trigger")
                    trigger_idx = find_trigger(trace_t, samprate, debug)
                    if trigger_idx is None:
                        print(f"recorder didn't find trigger in ch 2")
                        send_array(conn, np.array([]))
                        conn.sendall(b"FAIL CAP STOP\n")
                        continue
                    ...
                    send_array(conn, trace[trigger_idx:])
...
lastbuffer_t = None
def export_data(d, t):
    #print(f"{d.shape} {t.shape}")
    global lastbuffer, lastbuffer_t, exportrunning_onemore
    ...
    if len(exportbuffers) == 0:
        ...
        assert not (lastbuffer_t is None)
        exportbuffers.append(lastbuffer)
        exportbuffers_t.append(lastbuffer_t)
        lastbuffer = None
        lastbuffer_t = None
